...
You are helping to spin up a new feature spec for this application, from a short idea provided in the user input below. Always adhere to any rules or requirements set out in the `.github/copilot-instructions.md` file when responding.
...
This command runs in PLANNING MODE. Research read-only and produce ONLY the spec document and its git branch. Do NOT implement the feature, edit any existing application/source files, or run build, test, or other terminal commands. The only file you may create is the spec markdown described below.
...
Create a markdown spec document that Plan mode can use directly and save it in the _specs folder using the `feature_slug`. Use the exact structure as defined in the spec template file #SPEC.md located at the project root. Do not add technical implementation details such as code examples.
