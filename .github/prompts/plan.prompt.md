...
This command runs in PLANNING MODE. Research read-only and produce ONLY the technical plan document. Do NOT implement the feature, edit any existing application/source files, or run build, test, or other terminal commands. The only file you may create is the plan markdown in `_specs/`, named `<feature-name>_plan.md`.
...
- Run a single task with `/execute #<this-file> T<n>`, run every remaining task in order with `/execute #<this-file> ALL` (resumable — it skips tasks already checked), or implement the whole plan from a prompt like "implement the plan at #<this-file>".
