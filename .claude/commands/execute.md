---
description: Execute one task — or all remaining tasks — from a feature plan or spec file
argument-hint: "[plan or spec file] [task id e.g. T1, or ALL]"
allowed-tools: Read, Grep, Glob, Edit, Write, Bash, WebFetch(domain:raw.githubusercontent.com)
---

Execute work from the referenced feature plan or spec file — either a single task, or every remaining task in order. Always adhere to any rules or requirements set out in any CLAUDE.md files when responding.
...
1. `source_file`
   - The plan or spec file reference to read tasks from.
   - Example: `@_specs/new-heist-form_plan.md` or `@_specs/new-heist-form_spec.md`.

2. `task_id`
   - Either the identifier of a single task to execute (e.g. `T1`, `T2`, `T3.1`),
   - or the literal word `ALL` (case-insensitive) to execute every remaining task in order.
   - If neither is provided, list the available task ids and their checked/unchecked status from `source_file` and ask the user what to run. DO NOT guess or pick one yourself.
...
- Follow all project rules in CLAUDE.md and any referenced spec/plan conventions.
...
3. For each unchecked task, in order, one at a time:
   ...
   b. As soon as its acceptance criteria are met, immediately edit `source_file` to change that task's `- [ ]` to `- [x]` BEFORE starting the next task. Persisting progress after each task is what lets a later `/execute @<file> ALL` safely continue.
