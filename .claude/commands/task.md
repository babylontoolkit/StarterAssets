...
1. `source_file`
   ...
   - Example: `@_plans/new-heist-form/plan.md` or `@myfeature.md`.
...
3. For each unchecked task, in order, one at a time:
   ...
   b. As soon as its acceptance criteria are met, immediately edit `source_file` to change that task's `- [ ]` to `- [x]` BEFORE starting the next task. Persisting progress after each task is what lets a later `/task @<file> ALL` safely continue.
